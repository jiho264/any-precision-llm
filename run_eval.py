...
print(
    """This script will evaluate all models in the cache directory by:
...
for model_path in model_paths:
    ...
    datasets_with_results = [
        ...
        if any(
            testcase == key.split(":")[0]
            for key in all_results.get(model_name, {}).get("ppl", {}).keys()
        )
    ]
    tasks_with_results = [
        task
        ...
        if any(
            task == key.split(":")[0]
            for key in all_results.get(model_name, {}).get("lm-eval", {}).keys()
        )
    ]
    if not args.redo:
        model_jobs["ppl"] = [
            testcase for testcase in datasets if testcase not in datasets_with_results
        ]
        model_jobs["lm-eval"] = [
            task for task in tasks if task not in tasks_with_results
        ]
        ...
        if datasets_with_results:
            model_jobs["to_print"].append(
                f"Redoing all datasets, overwriting for {datasets_with_results}"
            )
        else:
            model_jobs["to_print"].append("No previous ppl results to overwrite.")
        if tasks_with_results:
            model_jobs["to_print"].append(
                f"Redoing all tasks, overwriting for {tasks_with_results}"
            )
        else:
...
total_ppl_job_count = sum(
    len(model_tasks["ppl"]) for model_tasks in total_tests_to_run.values()
)
total_lm_eval_job_count = sum(
    len(model_tasks["lm-eval"]) for model_tasks in total_tests_to_run.values()
)
if skipped_models:
    print(
        f">> {len(skipped_models)} models will be skipped because all dataset results already exist."
    )
    # print('\n'.join(skipped_models) + '\n')
print(
    f">> Summary: {total_ppl_job_count} ppl jobs and {total_lm_eval_job_count} lm-eval tasks"
...
for i, model_path in enumerate(total_tests_to_run):
    ...
    print("==================================================")
    datasets_with_results = [
        testcase for testcase in datasets if testcase in all_results.get(model_name, {})
    ]
    ...
    if lm_eval_results:
        ...
        all_results.setdefault(model_name, {}).setdefault("lm-eval", {}).update(
            lm_eval_results
        )
